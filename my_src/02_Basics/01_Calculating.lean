...
calc
  (a + b) * (a + b)
      = a * a + b * a + (a * b + b * b) :
    begin
      rw [add_mul, mul_add, mul_add, ← add_assoc, ← add_assoc, mul_comm b a]
    end
  ... = a * a + (b * a + a * b) + b * b : by rw [← add_assoc (a*a+b*a), add_assoc (a*a)]
  ... = a * a + 2 * (a * b) + b * b     : by rw [mul_comm b a, ← two_mul]

-- end the section (not the calc!)
end
...
calc
  (a + b) * (a - b) 
    ...
    ... = a*a - b*b                 : by rw [sub_self (a*b), add_zero]
